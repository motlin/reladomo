        ...
        extends MithraH2TestAbstract
{
            ...
            throws ParseException
    {
        ...
        String sql = "select " + getAllTypesColumns() + " from public.ALL_TYPES where TIMESTAMP_COL = '2007-01-01 01:01:01.999'";
        validateMithraResult(op, sql);
    }
...
//            System.out.println("took "+(System.currentTimeMillis() - start)+" ms");
    ...
    public void testTimestampRetrievalAcrossDaylightSavingTime() throws ParseException
    {
        IntHashSet expectedIds = IntHashSet.newSetWith(100, 200);
        this.insertTimestampConversion(100, "2000-01-01 00:30:00.000");
        this.insertTimestampConversion(200, "2000-07-01 00:30:00.000");
        MithraManagerProvider.getMithraManager().clearAllQueryCaches();

        Operation utcDay = TimestampConversionFinder.id().in(expectedIds)
                .and(TimestampConversionFinder.timestampValueUTC().dayOfMonth().eq(1));
        TimestampConversionList utcResults = TimestampConversionFinder.findMany(utcDay);
        assertEquals(expectedIds, TimestampConversionFinder.id().asGscSet(utcResults));

        Operation databaseDay = TimestampConversionFinder.id().in(expectedIds)
                .and(TimestampConversionFinder.timestampValueDB().dayOfMonth().eq(1));
        TimestampConversionList databaseResults = TimestampConversionFinder.findMany(databaseDay);
        assertEquals(expectedIds, TimestampConversionFinder.id().asGscSet(databaseResults));
    }

    private void insertTimestampConversion(int id, String timestampValue) throws ParseException
    {
        Timestamp timestamp = new Timestamp(timestampFormat.parse(timestampValue).getTime());
        TimestampConversion conversion = new TimestampConversion();
        conversion.setId(id);
        conversion.setTimestampValueUTC(timestamp);
        conversion.setTimestampValueDB(timestamp);
        conversion.setTimestampValueNone(timestamp);
        conversion.insert();
    }
